...
async function runStudentInterview(name,device){
  ...
  try {
    ...
    page.on('response',async r=>{
      if(!r.url().includes('/api/interview-turn'))return;
      try{apiResponses.push({status:r.status(),body:await r.json()})}catch{apiResponses.push({status:r.status(),body:await r.text()})}
    });
    await page.addInitScript(({cv})=>{
      localStorage.setItem('gjr_cv_context',cv);
      localStorage.setItem('gjr_cv_mode','general');
      localStorage.setItem('gjr_cv_file_context',cv);
      class FakeRecognition{
        start(){this.onstart?.();setTimeout(()=>this.onresult?.({resultIndex:0,results:[{0:{transcript:'At Coding Panda I worked on the compiler platform and built execution APIs.',confidence:0.99},isFinal:true}]}),250)}
        stop(){this.onend?.()}
      }
    ...
    pass(`${name}: production site loaded`);
    await click(page,/AI Audio Interview|Talk to your AI interviewer/i,`${name}: AI interview`);
    const body1=await page.locator('body').innerText();
    if(!/AI Mock Interview|LIVE INTERVIEW|real conversation/i.test(body1))throw new Error(`${name}: interview screen not reached`);
    ...
    const walk=(v,key='')=>{
      if(v==null)return;
      if(typeof v==='string'){
        if(/question|prompt|text/i.test(key)&&v.includes('?'))apiQuestions.push(v);
        return;
      }
    ...
    pass(`${name}: generated ${questions.length} visible/API question(s)`);
    info(`${name}: questions observed: ${questions.slice(0,8).join(' | ')}`);

    for(const q of questions){
      for(const bad of UNSUPPORTED){if(bad.test(q))fail(`${name}: unsupported CV claim in generated question: ${q}`)}
    }
    if(!questions.some(q=>SUPPORTED.some(re=>re.test(q)))){
      fail(`${name}: no generated question referenced any concrete CV-supported experience/project/technology`);
    } else {
      pass(`${name}: at least one generated question is concretely grounded in the student's CV`);
    }
    if(apiResponses.some(r=>r.status>=400))fail(`${name}: interview-turn returned HTTP error ${apiResponses.map(r=>r.status).join(',')}`);
    else if(apiResponses.length)pass(`${name}: interview-turn API returned successful response(s)`);
    else info(`${name}: no interview-turn response captured; question was produced client-side`);
...
fs.writeFileSync('student-cv-live-audit-report.json',JSON.stringify(report,null,2));
