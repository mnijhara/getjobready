(()=>{
  const WORKER='https://getjobready-ai-proxy.workers.dev';
  let mounted=false;
  ...
  function mount(){
    ...
    const title=[...document.querySelectorAll('.workspace h1')].find(x=>/Preparation Report/i.test(x.textContent||''));
    if(!workspace||!title)return;
    const start=[...workspace.querySelectorAll('button')].find(b=>/live audio interview/i.test(b.textContent||''));
    if(!start)return;
    ...
    const editor=studio.querySelector('[data-cv-editor]'),changes=studio.querySelector('[data-cv-changes]'),status=studio.querySelector('[data-cv-status]'),analyze=studio.querySelector('[data-cv-analyze]'),apply=studio.querySelector('[data-cv-apply]'),cont=studio.querySelector('[data-cv-continue]');
    let improved='';
    analyze.onclick=async()=>{status.textContent='AI is reviewing your CV…';analyze.disabled=true;try{const r=await fetch('/api/improve-cv',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({cv:editor.value,cvData:cvData(),cvMime:cvMime(),jd:jdText(),mode:mode()})});const d=await r.json();if(!r.ok)throw new Error(d.error||'Unable to improve CV');improved=d.improvedCV||editor.value;if(Array.isArray(d.changes))changes.innerHTML=d.changes.map(x=>`<li>${esc(x)}</li>`).join('');editor.value=improved;apply.disabled=false;status.textContent='AI improved draft loaded. Review and edit it before saving.';}catch(e){status.textContent=e.message||'CV improvement failed. Try again.';}finally{analyze.disabled=false}};
    apply.onclick=()=>{sessionStorage.setItem('gjr_cv_improved',editor.value);sessionStorage.setItem('gjr_cv_ready','1');apply.disabled=true;cont.disabled=false;cont.classList.remove('cv-locked');status.textContent='Saved. You are in control of the final CV. You can still edit it above.'};
    editor.addEventListener('input',()=>{sessionStorage.removeItem('gjr_cv_ready');cont.disabled=true;cont.classList.add('cv-locked');apply.disabled=false;status.textContent='Unsaved edits — save this version before interviewing.'});
    cont.onclick=()=>{sessionStorage.setItem('gjr_cv_improved',editor.value);sessionStorage.setItem('gjr_cv_ready','1');start.style.display='';start.click()};
  }
  new MutationObserver(()=>setTimeout(mount,20)).observe(document.body,{childList:true,subtree:true});
