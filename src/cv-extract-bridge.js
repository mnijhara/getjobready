  ...
  window.fetch = async (input, init = {}) => {
    ...
    if (url === '/api/extract-cv' && (init.method || 'GET').toUpperCase() === 'POST') {
      ...
      try {
        ...
        const r = await nativeFetch(`${WORKER}/generate`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: 'gemini-3.7-flash', contents: [{ parts: [{ text: prompt }, { inlineData: { mimeType: mime, data } }] }], generationConfig: { responseMimeType: 'text/plain', maxOutputTokens: 7000 } }) });
