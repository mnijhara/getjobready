...
window.fetch=async(input,init={})=>{
  ...
  if(path==='/api/ai-status' && (!init.method||init.method==='GET')){
    const r=await originalFetch(`${WORKER_URL}/health`),d=await r.json();
    return new Response(JSON.stringify({configured:!!d.ok,keySlots:Number(d.configuredKeys||0),healthySlots:Number(d.healthyKeys||0),model:d.model||'gemini-3.7-flash',router:d.router||'5-key round-robin + automatic failover',proxy:WORKER_URL}),{status:r.status,headers:{'Content-Type':'application/json'}});
  }
  if(path==='/api/analyze' || path==='/api/analyze-upload'){
    try{
      const body=typeof init.body==='string'?JSON.parse(init.body):(init.body||{}),mode=body.mode||'specific'; let prompt,parts;
      if(path==='/api/analyze-upload'){
        prompt=mode==='general'?`You are an expert campus recruiter and CV strategist. Read the attached CV WITHOUT assuming a target job. Return ONLY valid JSON with exactly: score (0-100), headline, summary, highlights (max 4), gaps (max 5), cvImprovements (max 5), rewrittenBullets (max 4 without inventing facts), plan (exactly 7 actionable steps), interviewQuestions (exactly 5 general interview questions grounded in this CV). Questions must cover actual projects, experience, achievements, teamwork, ownership, problem solving and behavioural readiness. Do not invent facts or assume a role.`:`You are an expert campus recruiter. Read the attached CV and analyse it against the target job description. Career mode: ${body.career||'job'}. Return ONLY valid JSON with exactly: score (0-100), headline, summary, highlights (max 4), gaps (max 5), cvImprovements (max 5), rewrittenBullets (max 4 without inventing facts), plan (exactly 7 actionable steps), interviewQuestions (exactly 5 role-specific questions). Target JD:\n${String(body.jd||'').slice(0,30000)}`;
        parts=[{text:prompt},{inlineData:{mimeType:body.mime||'application/pdf',data:body.data||''}}];
      }else{prompt=analysisPrompt(body.cv||'',body.jd||'',body.career||'job',mode);parts=[{text:prompt}]}
      const result=await generate(prompt,parts); return new Response(JSON.stringify(result),{status:200,headers:{'Content-Type':'application/json'}});
    }catch(error){return new Response(JSON.stringify({error:'AI service unavailable. Please try again.'}),{status:503,headers:{'Content-Type':'application/json'}})}
  }
  if(path==='/api/interview-turn'){
    try{
      const body=typeof init.body==='string'?JSON.parse(init.body):(init.body||{}),mode=body.mode||'specific',career=body.career||'job',turn=Math.max(1,Number(body.turn)||1),maxTurns=Math.min(7,Math.max(3,Number(body.maxTurns)||7));
      const context=mode==='general'?`GENERAL CV INTERVIEW. Candidate CV:\n${String(body.cv||'').slice(0,40000)}`:`ROLE-SPECIFIC CV + JD INTERVIEW. Target JD:\n${String(body.jd||'').slice(0,30000)}\nCandidate CV:\n${String(body.cv||'').slice(0,40000)}`;
      const history=Array.isArray(body.history)?body.history.slice(-8):[];
      const prompt=`You are a realistic campus-to-corporate voice interviewer for a ${career==='internship'?'summer internship':'full-time role'}. ${context}\n\nCurrent question: ${String(body.question||'').slice(0,1200)}\nCandidate answer: ${String(body.answer||'').slice(0,6000)}\nPrevious turns: ${JSON.stringify(history).slice(0,14000)}\nThis is turn ${turn} of ${maxTurns}.\nReturn ONLY valid JSON. If more interview is needed, return exactly: {"done":false,"nextQuestion":"...","evaluation":"brief evaluation of this answer","followUp":true|false}. Ask a natural follow-up when the answer is vague, unsupported or deserves probing; otherwise progress to the next competency. Use the CV/JD evidence and never invent candidate facts. If turn ${turn} is the final turn, return exactly: {"done":true,"finalFeedback":{"score":0,"strengths":[...],"improvements":[...],"nextAction":"..."}}. Score 0-100 and evaluate clarity, structure, evidence, ownership, relevance, communication and business thinking.`;
      const result=await generate(prompt); return new Response(JSON.stringify(result),{status:200,headers:{'Content-Type':'application/json'}});
    }catch(error){return new Response(JSON.stringify({error:'Interview AI service unavailable.'}),{status:503,headers:{'Content-Type':'application/json'}})}
  }
