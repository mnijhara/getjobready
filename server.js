...
app.post("/api/extract-cv", async (req, res) => {
  ...
  if (mime === "application/pdf") {
    try {
      ...
      const clean = out.replace(/(\b[A-Za-z]{2,})-\s+([a-z]{2,}\b)/g, "$1$2").replace(/\bTechnolo(?:\.|\b)(?!\w)/gi, "Technology").replace(/\bEngin(?:\.|\b)(?!\w)/gi, "Engineering").replace(/\s+([,;.])/g, "$1").trim();
...
${cv ? String(cv).slice(0, 4e3) : "General student / early career candidate"}
...
- nextQuestion: (string) A proactive follow-up question to help them deepen their AI skills.`;
  try {
    const data = await generate(prompt, { maxOutputTokens: 2e3 });
...
- done: (boolean) true if the situation reached an agreed resolution, false if another turn is needed.`;
  try {
    ...
    const replyText = acknowledges ? "I appreciate you addressing this head-on. Let's make sure the revised numbers are updated and verified. Please send me the updated draft before 3:45 PM so we can do a final review before leadership joins." : "I hear your explanation, but right now the client partner is walking into that room in 45 minutes. What is the immediate contingency plan to get these deliverables client-ready?";
