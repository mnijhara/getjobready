...
const allowedOrigin = process.env.PUBLIC_BASE_URL ? process.env.PUBLIC_BASE_URL.replace(/\/$/, '') : null;
...
app.use('/api', (req,res,next)=>{if(req.path==='/health'||req.path==='/ai-status')return next();const now=Date.now();const key=req.ip||req.socket.remoteAddress||'unknown';const bucket=rateBuckets.get(key)||{start:now,count:0};if(now-bucket.start>60000){bucket.start=now;bucket.count=0;}bucket.count+=1;rateBuckets.set(key,bucket);if(bucket.count>45)return res.status(429).json({error:'Too many requests. Please try again shortly.'});next();});
