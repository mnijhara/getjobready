...
const fallback = {
  ...
  cvImprovements: ['Lead bullets with action + outcome', 'Quantify scope, impact or scale wherever possible', 'Move the most relevant skills and projects higher'],
  rewrittenBullets: ['Led a project that improved a measurable business outcome by using a structured approach.', 'Collaborated with a cross-functional team to deliver a project within the agreed timeline.'],
...
app.post('/api/interview-feedback', async (req, res) => {
  ...
  try {
    const result = await generate(`You are a demanding but supportive campus interviewer. Evaluate these interview answers against the target JD. Return ONLY valid JSON with exactly: score (0-100), strengths (max 4), improvements (max 4), nextAction. Assess clarity, structure, evidence, ownership, relevance, communication, confidence and business thinking. Give specific actionable feedback, not generic advice.\n\nTARGET JD:\n${jd}\n\nANSWERS:\n${JSON.stringify(answers)}`);
    return res.json(result);
  } catch (error) {
...
app.post('/api/coach', async (req, res) => {
  ...
  const prompt = module === 'ai'
    ? `Create a practical AI-at-work learning sprint for a student entering a ${career === 'internship' ? 'summer internship' : 'corporate role'}. Focus on research, writing, analysis, meetings, automation, verification and responsible use. Return ONLY JSON with diagnosis, workflows (5), promptExamples (5), guardrails (5), sevenDayPlan (7). Context: ${context}`
  ...
  try { return res.json(await generate(prompt)); }
...
app.post('/api/demo', async (req, res) => {
  ...
  try {
    return res.json(await generate(`You are a product strategist helping a student impress a corporate interviewer. Analyse the stated company problem and create a credible product concept. Return ONLY valid JSON with: title, tagline, users, impact, pitch (array of 4 bullets), html. The html must be a complete self-contained polished HTML document, inline CSS only, responsive, no external assets, and suitable to open as a live demo. Company: ${company}. Problem: ${problem}. Candidate idea: ${idea}`));
  } catch (error) {
