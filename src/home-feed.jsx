function installCareerFeed(){
  ...
  <section class="career-strip"><button type="button" class="${activeCareer==='internship'?'active':''}" data-feed-career="internship">☀️ Internship</button><button type="button" class="${activeCareer==='job'?'active':''}" data-feed-career="job">💼 Full-time</button></section>
  ...
  <div class="action-feed">
    <button type="button" class="feed-card purple gjr-resume"><div class="feed-icon">📄</div><div><span class="feed-tag">START HERE</span><h3>Get your CV matched</h3><p>Review your CV generally or match it to one specific JD.</p></div><b>→</b></button>
  ...
  <section class="daily-card"><div><span class="eyebrow">TODAY'S MISSION</span><h3>Give me your 90-second answer.</h3><p>Practice your introduction before your next interview.</p></div><button type="button" class="round-btn gjr-interview">▶</button></section>
  <nav class="feed-nav" aria-label="Student navigation">
    <button type="button" class="active" data-feed-nav="home"><strong>⌂</strong><span>Home</span></button>
    <button type="button" data-feed-nav="prepare"><strong>▣</strong><span>Prepare</span></button>
    <button type="button" data-feed-nav="interview"><strong>◉</strong><span>Interview</span></button>
    <button type="button" data-feed-nav="progress"><strong>✦</strong><span>Progress</span></button>
  </nav>`;
  home.replaceWith(feed);
  grid.style.display='none';
  root.querySelector('.roadmap')?.remove();
  const clickModule=(index)=>{
    const buttons=[...root.querySelectorAll('.module-grid .module-card')];
    buttons[index]?.click();
  };
