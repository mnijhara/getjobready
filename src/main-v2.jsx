...
function App(){
 ...
 if(screen==='interview')return <Workspace title="AI Audio Interview" subtitle={prep==='general'?'General interview grounded in your final CV.':'Role-specific interview grounded in your final CV + JD.'} icon={<Mic/>} back={()=>setScreen('cvstudio')} onHome={goHome}><VoiceInterview cv={cv}jd={jd}mode={prep}career={career}roleName={roleName}question={questions[qIndex]||'Tell me about a key achievement from your CV.'}turn={qIndex+1}maxTurns={questions.length||6}history={answers}onTurn={(d,a)=>{setAnswers(x=>[...x,{question:questions[qIndex],answer:a,evaluation:d?.evaluation}]);if(!d.done){if(d.nextQuestion&&!questions.includes(d.nextQuestion)){setResult(r=>({...r,interviewQuestions:[...(r?.interviewQuestions||questions),d.nextQuestion]}))};setQIndex(i=>i+1)}}}onDone={saveInterviewResult}/></Workspace>;
