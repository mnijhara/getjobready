...
// Backward-compatible single-key support for staging/testing.
if (!slots.length && process.env.GEMINI_API_KEY) {
  slots.push({ index: 0, key: process.env.GEMINI_API_KEY, failures: 0, cooldownUntil: 0, requests: 0, lastUsed: 0 });
}
...
function publicStatus() {
  return {
    ...
    healthySlots: slots.filter(s => Date.now() >= s.cooldownUntil).length,
    model: DEFAULT_MODEL,
    router: 'round-robin + automatic failover',
  };
}
...
function nextSlot() {
  ...
  for (let i = 0; i < slots.length; i++) {
    const pos = (cursor + i) % slots.length;
    const slot = slots[pos];
    if (now >= slot.cooldownUntil) {
      cursor = (pos + 1) % slots.length;
      slot.lastUsed = now;
      slot.requests += 1;
      return slot;
    }
  }
  // If every slot is cooling down, use the one that becomes available first.
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        markFailure(slot, response.status);
        lastError = new Error(`Gemini ${response.status}: ${body.slice(0, 300)}`);
        continue;
      }
      ...
      lastError = error;
      if (!String(error.message || '').includes('Gemini 4')) markFailure(slot, 500);
    }
  }
