...
const safeEvaluator = `function evaluateInterviewTurnLocal(question,answer,history){
 ...
 const words=cleanAns.split(/\\s+/).filter(Boolean); const wordCount=words.length;
 ...
 if(wordCount<=3||isGibberish||cleanAns.length<12){turnScore=0;note='0/100 — The answer is too short or generic. Give a specific example from your CV and explain what you personally did and what happened.';}
  ...
  const hasResult=/\\b(result|outcome|impact|improved|reduced|increased|achieved|delivered|learned|success)\\b|%|\\b\\d+\\b/i.test(cleanAns);
  turnScore=hasOwnership&&hasResult?85:(hasOwnership||hasResult?70:55);
  note=turnScore>=85?'Strong STAR answer. You explained your contribution and outcome clearly.':turnScore>=70?'Good detail. Make your personal contribution and actual outcome even clearer.':'Add a real CV example and structure it as Situation → Task → Action → Result.';
 }
 const fillerMatches=cleanAns.match(/\\b(um|uh|er|ah|like|you know|basically|actually|literally)\\b/gi)||[];
 const fillers=fillerMatches.length; const fillerList=[...new Set(fillerMatches.map(f=>f.toLowerCase()))];
 if(fillers)note+=` (${fillers} verbal crutch${fillers>1?'es':''} detected.)`;
 ...
 const prior=Array.isArray(history)?history:[]; const allTurns=[...prior,{question:q,answer:cleanAns,evaluation:{score:turnScore}}];
 const avgScore=Math.round(allTurns.reduce((sum,t)=>sum+(t.evaluation?.score??0),0)/allTurns.length);
 const improvements=[]; if(turnScore<70)improvements.push('Use a real CV example and answer with Situation → Task → Action → Result.'); improvements.push('Only state facts, technologies and outcomes supported by the CV or question context.');
