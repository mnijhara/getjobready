...
function App(){
 ...
 const handleLogin=email=>{db.saveProfile(email);setProfile({email});db.syncServerData(email).then(()=>{setCv(db.getMasterCV());setProfile(db.getProfile())}).catch(()=>{});setJd('');setAppId(null);setRoleName('');scrollToTop();setScreen('home');db.syncFromCloud().catch(()=>{})};
