...
expect(app.includes("saveSession('gjr_cv_text',cleaned)") || app.includes("saveSession('gjr_cv_text',text)"), 'uploaded CV text is persisted');
expect(app.includes("saveSession('gjr_jd_text',prep==='general'?'':jd)"), 'target JD text is persisted after analysis');
expect(app.includes("saveSession('gjr_cv_improved',finalText)") || app.includes("saveSession('gjr_cv_improved',text)"), 'saved improved CV is persisted');
