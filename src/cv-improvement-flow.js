(()=>{
  ...
  function mount(){
    if(mounted)return;
    const workspace=document.querySelector('.workspace'),title=[...document.querySelectorAll('.workspace h1')].find(x=>/Preparation Report/i.test(x.textContent||''));
    ...
    const markDirty=()=>{cont.disabled=true;save.disabled=!editor.value.trim();setStatus('Unsaved edits. Save your final CV before starting the interview.')};
