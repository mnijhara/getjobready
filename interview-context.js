(()=>{
  ...
  window.fetch=async(input,init={})=>{
    ...
    if((url.includes('/api/analyze-upload')||url.includes('/api/analyze')||url.includes('/api/interview-turn'))&&init?.method==='POST'){
      try{
        ...
        const selected=localStorage.getItem(keyMode)||'specific'; body.mode=selected;
        const cvText=document.querySelector('#cvText')?.value, jdText=document.querySelector('#jdText')?.value;
        ...
        if(url.includes('/api/analyze-upload')){
          ...
          if(!body.data){const cv=sessionStorage.getItem(keyText)||'';if(cv){body.cv=cv;return originalFetch('/api/analyze',{...init,body:JSON.stringify(body)});}}
        }else if(url.includes('/api/analyze')){
          body.cv=body.cv||sessionStorage.getItem(keyText)||''; body.jd=selected==='general'?'':(body.jd||sessionStorage.getItem(keyJd)||'');
        }else{
          body.mode=localStorage.getItem(keyInterview)||selected; body.cvData=sessionStorage.getItem(keyData)||''; body.cvMime=sessionStorage.getItem(keyMime)||''; body.cv=sessionStorage.getItem(keyText)||body.cv||''; body.jd=body.jd||sessionStorage.getItem(keyJd)||'';
        }
  ...
  const remember=()=>{
    const t=document.querySelector('#cvText'); if(t&&!t.dataset.gjrContext){t.dataset.gjrContext='1';const save=()=>sessionStorage.setItem(keyText,t.value||'');t.addEventListener('input',save);if(t.value)save();}
    const j=document.querySelector('#jdText'); if(j&&!j.dataset.gjrContext){j.dataset.gjrContext='1';const save=()=>{if(!j.value.startsWith('GENERAL CV REVIEW'))sessionStorage.setItem(keyJd,j.value||'');};j.addEventListener('input',save);if(j.value&&!j.value.startsWith('GENERAL CV REVIEW'))save();}
  };
  function interviewChooser(){
    ...
    m.addEventListener('click',e=>{if(e.target===m||e.target.closest('[data-close]'))return m.remove();const b=e.target.closest('[data-interview]');if(!b)return;localStorage.setItem(keyInterview,b.dataset.interview);localStorage.setItem(keyMode,b.dataset.interview);m.remove();setTimeout(()=>document.querySelector('#startStory')?.click(),0);});
  }
  document.addEventListener('click',e=>{
    ...
    if(analyze&&(localStorage.getItem(keyMode)||'specific')==='general'){
      const jd=document.querySelector('#jdText');if(jd&&!jd.value){jd.value='GENERAL CV REVIEW — no target job description';jd.dispatchEvent(new Event('input',{bubbles:true}));}
    }
  },true);
  new MutationObserver(remember).observe(document.documentElement,{childList:true,subtree:true}); remember();
})();
