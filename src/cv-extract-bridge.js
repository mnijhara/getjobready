  ...
  window.fetch = async (input, init = {}) => {
    ...
    if (url === '/api/extract-cv' && (init.method || 'GET').toUpperCase() === 'POST') {
      ...
      if (body && typeof body === 'object' && body.data) {
        ...
        if (normalizedMime !== body.mime) init = { ...init, body: JSON.stringify({ ...body, mime: normalizedMime }) };
      }
      try {
      ...
      try {
        ...
        const r = await nativeFetch(`${WORKER}/generate`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt, model: 'gemini-3.7-flash', contents: [{ parts: [{ text: prompt }, { inlineData: { mimeType: mime, data } }] }], generationConfig: { responseMimeType: 'text/plain', maxOutputTokens: 7000 }, json: false }) });
        if (!r.ok) { const err = await r.text().catch(() => ''); return new Response(JSON.stringify({ error: `CV extraction service ${r.status}: ${err.slice(0, 240)}` }), { status: r.status, headers: { 'Content-Type': 'application/json' } }); }
      ...
      } catch (e) { return new Response(JSON.stringify({ error: `CV extraction bridge failed: ${String(e?.message || e)}` }), { status: 503, headers: { 'Content-Type': 'application/json' } }); }
    }
